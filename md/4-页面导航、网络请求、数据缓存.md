...
![img](https://cdn.jsdelivr.net/gh/mqxu/wiki-image@master/uPic/eee9ff1dcdaf73b20fa502e834e1083f_1224x354.png)
...
如果你要用 Uniapp 开发的项目编译成微信小程序，就要注意微信小程序的页面栈的限制了，小程序中页面栈**限制最多十层**（微信进行了限制调整），随着页面栈的push增加，在不知道的情况下就会堆栈到十层，再用 API navigateTo 去跳转页面就跳不动了，用户会跳转失效（卡死状态）。
...
![img](https://img.kancloud.cn/4e/7e/4e7e704d4544685f7e312c2b0cfab809_597x358.png)
...
很多场景下是如果请求2个接口数据，当某一个接口成功的时候，就可能需要禁止掉另外一个接口的继续请求了：
...
上面对 request 对象进行一次返回，这样我们可以调用该对象下的 `abort` 方法，可中断请求任务。如果需要更好的实现拦截，并统一管理请求，Uniapp 插件市场有 flyio、axios 等三方封装的拦截器可用。
...
1. 良好体验的 APP，还会判断当前是否处于飞行模式、是 wifi 还是 蜂窝数据
2. 单次网络请求数据量建议控制在50K以下（仅指 JSON数据，不含图片），过多数据应分页获取，以提升应用体验。
...
4. 微信小程序**单个 key 允许存储的最大数据长度为 1MB，所有数据存储上限为 10MB**。
