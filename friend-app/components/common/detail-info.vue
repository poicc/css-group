	...
	<view class="p-2 animated fadeInLeft fast">
		<view class="flex mb-2">
			...
			<view v-if="!item.isFollow" @tap="follow(item)"
				class="icon iconfont icon-zengjia bg-blue text-white rounded w-10 h-6 px-1">关注</view>
			<view v-else class="icon iconfont border border-blue text-blue rounded w-10 h-6 px-1">已关注</view>
		</view>
		<!-- 标题和缩略图部分 -->
		<view class="relative">
			<!-- 有缩略图就显示缩略图，没有就用默认图 -->
			<image :src="item.thumbnail" class="w-71 h-40 rounded" v-if="item.thumbnail !==''"></image>
			<image src="/static/img/avatar/avatar1.jpg" class="w-71 h-40 rounded" v-else></image>
			<!-- 文章标题 -->
			<view class="text-white text-xl mb-2 absolute" style="bottom: 40rpx;left:20rpx;">
				{{item.title}}
			</view>
			<!-- 遮罩层 -->
